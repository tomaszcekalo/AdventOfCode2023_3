...
if (new GearRatioEvaluator(inputSmall).Evaluate2() != 4361)
{
    throw new Exception("Wrong");
}
if (new GearRatioEvaluator(inputSmall2).Evaluate2() != 413)
{
    throw new Exception("Wrong");
}
if (new GearRatioEvaluator(inputSmall3).Evaluate2() != 925)
{
    throw new Exception("Wrong");
}
if (new GearRatioEvaluator(inputSmall4).Evaluate2() != 4)
{
    throw new Exception("Wrong");
}
if (new GearRatioEvaluator(inputSmall5).Evaluate2() != 156)
{
    throw new Exception("Wrong");
}
Console.WriteLine("Evaluation: ");
var sum = new GearRatioEvaluator(input).Evaluate2();
Console.WriteLine($"SUM = {sum}");

public class GearRatioEvaluator
{
    private IEnumerable<char>? _symbols;// = inputSmall.Where(x => !char.IsDigit(x) && x != '.');
    private string[] _lines;

    public GearRatioEvaluator(string input)
    {
        _lines = input.Split(Environment.NewLine);
        _symbols = _lines.SelectMany(x => x).Where(x => !char.IsDigit(x) && x != '.').Distinct();
    }
    ...
    public int Evaluate2()
    {
        ...
        for (int y = 0; y < _lines.Length; y++)
        {
            ...
            for (int x = 0; x < _lines[y].Length; x++)
            {
                ...
                else
                {
                    if (amount > 0)
                    {
                        ...
                        bool isThereASymbol = IsThereSymbol(x, y, startX);
                        if (isThereASymbol)
                        {
            ...
            if (amount > 0)
            {
                ...
                bool isThereASymbol = IsThereSymbol(_lines[y].Length - 1, y, startX);
                if (isThereASymbol)
                {
                    Console.Write($"{number}, ");
                    sum += number;
    ...
    public bool IsThereSymbol(int xEnd, int row, int startX)
    {
        for (int y = int.Max(row - 1, 0);
            y <= int.Min(row + 1, _lines.Length -1);
            y++)
        {
            for (int x = int.Max(startX - 1, 0);
                x <= int.Min(xEnd , _lines[y].Length -1);
                x++)
            {
                //Console.WriteLine($"{y} {x} {_lines[y][x]}");
                if (_symbols.Contains(_lines[y][x]))
                {
                    //Console.Write(_lines[y][x]);
                    return true;
                }
            }
        }
    ...
    public int GetNumber(string item, int y)
    {
        var numbers = item
            .Split(_symbols.ToArray(), StringSplitOptions.RemoveEmptyEntries);

        if (item.Any(x => _symbols.Contains(x)))
        {
        ...
        else
        {
            ...
            {
                var x = _lines[y].IndexOf(item);
                if (y > 0)
                {
                    for (int i = 0; i < item.Length; i++)
                    {
                        if (_symbols.Contains(_lines[y - 1][int.Max(0, x - 1 + i)])
                            || _symbols.Contains(_lines[y - 1][x + i])
                            || _symbols.Contains(_lines[y - 1][int.Min(x + 1 + i, _lines[int.Min(x + 1 + i, _lines.Length - 1)].Length - 1)]))
                        {
    ...
    public int Evaluate()
    {
        ...
        for (int y = 0; y < _lines.Length; y++)
        {
            var items = _lines[y].Split('.');

            foreach (var item in items)
            {
                if (item.Any(x => char.IsDigit(x)))
                {
                    var number = GetNumber(item, y);
                    sum += number;
                }
            }
        }
